...
import { X, Wand2, User, Mail, Phone, Globe, FileText, Sparkles, Calendar, Home, Megaphone } from 'lucide-react';
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-4 relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
                ...
                <Input placeholder="e.g. 123 Ocean Blvd" value={formData.propertyAddress} onChange={(e) => setFormData({...formData, propertyAddress: e.target.value})} />
            </div>
            <div className="space-y-1.5">
                ...
                <Input placeholder="e.g. Summer Penthouses FB" value={formData.campaignSource} onChange={(e) => setFormData({...formData, campaignSource: e.target.value})} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                  ...
                  <select 
                      className="flex h-11 w-full rounded-xl border border-slate-200 bg-slate-50/50 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all focus:bg-white"
              ...
              <Input label="Task Due Date" type="datetime-local" value={formData.taskDueDate} onChange={(e) => setFormData({...formData, taskDueDate: e.target.value})} />
          </div>
          <div className="relative group">
            <div className="flex justify-between items-center mb-1.5">
                <label className="text-sm font-medium text-slate-700 ml-1">Discovery Notes</label>
                <button type="button" onClick={generateEnrichment} disabled={isGenerating} className="text-[10px] font-black uppercase tracking-widest text-indigo-600 bg-indigo-50 px-2 py-1 rounded-full hover:bg-indigo-100">
                    {isGenerating ? <Sparkles className="w-3 h-3 mr-1 animate-spin" /> : <Wand2 className="w-3 h-3 mr-1" />}
                    AI Enrichment
                </button>
            </div>
            <textarea className="flex w-full rounded-xl border border-slate-200 bg-slate-50/50 px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 min-h-[80px] resize-none" placeholder="Notes on budget, location preference, or family size..." value={formData.notes} onChange={(e) => setFormData({...formData, notes: e.target.value})} />
          </div>
          <div className="pt-4 flex justify-end gap-3">
            <Button type="button" variant="ghost" onClick={onClose}>Cancel</Button>
            <Button type="submit" className="bg-slate-900 text-white px-8">Save Profile</Button>
          </div>
        </form>
      </div>
    </div>
